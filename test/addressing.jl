using Random
import Actors: AddressBook, Addr

@testset "Addressing" begin
    rng = MersenneTwister(33)
    book = AddressBook()

    @test_throws BoundsError book[Addr(1)]
    @test book.live[1].readers[] == 0
    @test book.live[2].readers[] == 0
    @test book.flips[] == 0

    null_id = Addr(UInt32(0))
    a = Actors.Actor{Any}(1, null_id)
    @test Addr(1) == push!(book, a)
    @test book.flips[] == 1
    ...
    Threads.@threads for i in rand(rng, 1:2000, 10000)
        ...
        @assert book[Addr(i)].state != nothing
    end
